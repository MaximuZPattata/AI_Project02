...
std::vector<cEnemyAttributes> enemyDetailsList;

const float POSITION_OFFSET = 0.0f;

extern void InitializingEnemyAIs();
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for(int index = 0; index < enemyDetailsList.size(); index++)
            enemyDetailsList[index].Wander(gameEngine);
